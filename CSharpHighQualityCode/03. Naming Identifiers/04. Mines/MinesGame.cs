namespace _04.Mines
{
    ...
    public class MinesGame
    {
        ...
        private static void DisplayMineCountOnEmptyFields(char[,] playfield)
        {
            ...
            for (int i = 0; i < col; i++)
            {
                for (int j = 0; j < row; j++)
                {
                    if (playfield[i, j] != MineField)
                    {
                        char totalMines = CountMines(playfield, i, j);
                        playfield[i, j] = totalMines;
                    }
                }
            }
        }
